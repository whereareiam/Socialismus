...
public class SwapperModule implements Module {
		...
		private SwapperConfig createExampleSwapperConfig() {
				...
				Swapper clickableLink = injector.getInstance(Swapper.class);
				clickableLink.placeholders.add("$https?:\\\\/\\\\/[^\\\\s]+");
				clickableLink.content.add("<red><click:OPEN_URL:{matched}>{matched}</click></red>");
				clickableLink.settings.directSearch = false;
				clickableLink.settings.randomContent = true;
				clickableLink.requirements.enabled = false;
				clickableLink.requirements.usePermission = "";

				swapperConfig.swappers.add(clickableLink);
